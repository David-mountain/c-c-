...
int j1 = s1; // j1=0xffff8765 (短 -> 长) 有符号位补充1 即f
int j2 = s2; // j2=0x00008765 (短 -> 长) 无符号补充0
...
#define 
const 
enum 较流行，语法类似struct  中间逗号隔开 若未指定，默认0开始，若指定了，则依次+1 	
...
```c
	...
	| 按位或
     ...
     ^ 按位异或
      ...
      与0异或得原，与1异或得反。
      应用：
      异或用来判断两个相等位是否为异，为异则1，为同则0. -> a^a=0
      1. 与1相^，使特定为翻转
      2. 与0相^，保留原值
      3. 交换两个值，不用临时变量
...
作输出型参数！！！一定要记住
遍历栈内指针数组 但是不可二级指针遍历二维数组？可以操控
指针数组做参数，实际上编辑器内部是二级指针
int **ppa = &pa;
...
二维数组 指针数组 二级指针：
a)二维数组不可以直接给二级指针赋值，   二维数组的形参是行指针？还有具体类型？
b 把二维数组每个行的地址赋给指针数组是可以的。
c  可以把指针数组的地址赋给二级指针。
```
...
```c
整型数组 字符数组 指针数组 结构体数组

一维数组
	1. 一维数组做实参，形参如何写
	2. 变量接收一维数组
	char sNames[5] = { 'a','b','c','d','\0' };
	auto pa = &sName;
	char *ps = sName;
	char (*p)[5] = &sName; // 和二维数组的一行是一个道理
	
二维数组
	二维数组做参数，内部已经转成了行指针
	void Test (int a[][3]) -> void Test (int (*a)[3])
  	也就是第一行的地址，类比第一个元素的地址，二维数组第一个元素就是第一行嘛
  	行指针常用于在参数，代入作为实参的二维数组
	1. 二维数组做实参
	void test2arr (int a[][3]) // int (*a)[3]
    {
        printf("sizeof(a) = %lld \n", sizeof(a)); // 8 指针大小
        printf("sizeof(*a) = %lld \n", sizeof(*a)); // 12 二维数组的第一个元素 第一行
        printf("sizeof(a[1]) = %lld \n", sizeof(a[1])); // 12 同上
    }
	2. 变量接收二维数组
	double a[5][12];
	double (*q)[12] = a;
...
指针数组
		最常用于多个字符串的集合
	  	char* strs[] = { "abd", "eig", "dgti" };
	  	只读，不可写  char* p = strs[1];  char c = p[0];
	  	也可用于链表内的每个节点地址记录下来，然后在数组内排序再打印出来等等
	  	应用：DOS参数 main函数形参argv就是指针数组 至少有2条指针
	  	最后一条指针是空指针(0),其他的指针都是指向堆区
		第一条指针指向字符串是执行文件名，其他字符串是DOS参数。
    1）指针数组： int * p1[10]
